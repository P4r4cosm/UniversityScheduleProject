...
public class ScheduleRepository
{
    ...
    public List<Schedule> GetSchedulesByLecturesIdsAndGroupId(IEnumerable<Lecture> lecturesIds,int groupId)
    {
        var lecturesIdsList = lecturesIds.Select(l => l.Id);
        var res=  _context.Schedules
            .Where(s=>lecturesIdsList.Contains(s.LectureId) && s.GroupId == groupId)
            .ToList();
        return res;
    }
}